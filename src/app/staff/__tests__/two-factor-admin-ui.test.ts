...
test("staff UI shows 2FA status and hides issue/reset for the current administrator", () => {
  ...
  assert.match(crm, /Cod emis/);
  assert.match(crm, /Neconfigurat/);
  assert.match(crm, /user\.id !== currentUserId/);
  assert.match(crm, /IssueTwoFactorActivationDialog/);
  assert.match(crm, /ResetTwoFactorDialog/);
  assert.match(dialogs, /export function IssueTwoFactorActivationDialog/);
  assert.match(dialogs, /export function ResetTwoFactorDialog/);
  assert.match(dialogs, /Emite cod 2FA/);
  assert.match(dialogs, /name="confirmation"/);
  assert.match(dialogs, /toate sesiunile/i);
  assert.match(dialogs, /Comunică acest cod separat, ideal personal/);
  assert.match(dialogs, /nu mai poate fi afișat/);
  assert.match(dialogs, /navigator\.clipboard\.writeText/);
  assert.match(dialogs, /activationExpiresAt/);
});
